...
def run_algorithm(alg, grid):
    """
        The function run_algorithm(alg, grid)
        calls one of the primary algorithms,
    ...
    # Random Connect or Heatmap
    if len(alg) is 1:
        algorithms[alg][1](grid)

    elif len(alg) is 2:
        # Battery Cycler, K-Means or Bat Propagation
        if type(alg[1]) is int:
            if alg[0] is 'b':
                ...
                pop = getInt()
                grid.update(algorithms[alg[0]][1](grid,
                                                  p_size=pop, generations=gen))
                random_battery_cycler(grid, alg[1])

        # Migration with all combos
